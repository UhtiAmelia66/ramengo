...
<section class="relative mx-auto max-w-7xl px-6 py-8">
    <div class="text-center mb-8">
        <p class="uppercase tracking-[0.25em] text-amber-200 font-bold">
            RamenGo News Room
        </p>
        <h1 class="mt-3 text-4xl md:text-5xl font-black bg-gradient-to-r from-amber-200 via-yellow-400 to-amber-600 bg-clip-text text-transparent">
            📰 Berita & Informasi
        </h1>
    </div>
    ...
    <div class="grid gap-8 xl:grid-cols-2">
        <section class="space-y-6">
            ...
            <div class="grid gap-5 sm:grid-cols-1">
                @forelse($eventContents as $content)
                    <article class="group overflow-hidden rounded-3xl shadow-2xl hover:-translate-y-1 transition duration-300">
                        <div class="relative overflow-hidden">
                            @if($content->poster_url)
                                <img
                                    src="{{ $content->poster_url }}"
                                    class="w-full h-[240px] object-cover duration-500 group-hover:scale-105"
                                    alt="{{ $content->judul }}"
                                >
                            @else
                                <div class="flex w-full h-[240px] items-center justify-center bg-[#1e1310] px-6 text-center">
                                    <h3 class="text-2xl font-black text-amber-300">{{ $content->judul }}</h3>
                                </div>
                            @endif
                            ...
                            <div class="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black via-black/60 to-transparent p-4">
                                ...
                                <button
                                    type="button"
                                    onclick="openNewsModal('{{ $content->id }}')"
                                    class="mt-3 bg-[#D4AF37] text-black font-bold px-4 py-2 rounded-xl"
                                >
                                    Detail
                                </button>
                            </div>
                ...
                @empty
                    <div class="rounded-3xl border border-amber-300/20 bg-black/30 p-8 text-center">
                        <p class="text-lg font-bold text-amber-100">
        ...
        <aside class="space-y-6">
            ...
            <div class="grid gap-5 sm:grid-cols-1">
                @forelse($promoContents as $content)
                    <article class="group overflow-hidden rounded-3xl shadow-2xl hover:-translate-y-1 transition duration-300">
                        <div class="relative overflow-hidden">
                            @if($content->poster_url)
                                <img
                ...
                @empty
                    <div class="rounded-3xl border border-yellow-300/20 bg-black/30 p-8 text-center">
                        <p class="text-lg font-bold text-yellow-100">
...
@foreach($contents as $content)
    <div
        id="modal-data-{{ $content->id }}"
        class="hidden"
        data-judul="{{ $content->judul }}"
        data-gambar="{{ $content->poster_url ?? '' }}"
...
<div id="newsModal" class="hidden fixed inset-0 z-50 bg-black/90 items-center justify-center p-5">
    <div class="bg-[#140908] rounded-3xl overflow-hidden max-w-5xl w-full grid md:grid-cols-2">
        <div class="min-h-[280px] bg-black/40">
            <img
                id="modalImage"
                class="hidden w-full h-full min-h-[280px] object-cover"
                alt="Detail Poster"
            >
        </div>
        ...
        <div class="p-8">
            ...
            <div id="modalContent" class="mt-6 leading-8 text-[#F4EFEA]/85"></div>

            <button
                type="button"
                onclick="closeNewsModal()"
                class="mt-6 bg-[#D4AF37] text-black font-bold px-6 py-3 rounded-xl"
            >
                Tutup
            </button>
        </div>
...
<script>
    function openNewsModal(id) {
        ...
        if (data.dataset.gambar) {
            modalImage.src = data.dataset.gambar;
